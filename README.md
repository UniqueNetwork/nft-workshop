...
Second, configure the image path here in `create_collection.js` file (replace `<your IPFS folder hash>` with your IPFS folder hash). The offchain schema can be changed later because offchain files may change their location:
```
